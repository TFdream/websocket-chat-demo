...
import websocket.springboot.handler.BizProcessHandler;
import websocket.springboot.handler.interceptor.AuthHandshakeInterceptor;
...
public class WebSocketConfig implements WebSocketConfigurer {
    private final int MAX_MESSAGE_BUFFER_SIZE = 1024 * 128;
    ...
    public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
        ...
        registry.addHandler(bizProcessHandler(), "/endpoint")
                .addInterceptors(authHandshakeInterceptor())
                .setAllowedOrigins("*");
    }

    @Bean
    public WebSocketHandler bizProcessHandler() {
        return new BizProcessHandler();
    }
